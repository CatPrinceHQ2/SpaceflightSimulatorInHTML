...
- **Interstellar travel / Alpha Centauri** There's a second star system using the binary pair hooks i made previously, specifically Alpha Centauri A and B, orbiting their barycenter way off from the Sun, plus Proxima Centauri (with 2 planets, Proxima b and Proxima d) further out orbiting that same barycenter. Reuses the same shit (`binaryAnchorPos`/`binaryPairPosition`) that already handled Pluto/Charon. This is where that 5th soundtrack "Night Vision" plays btw. It's a banger and ofc it slaps.
...
A **Cheats** panel (build and flight) exposes developer/debug tools: infinite fuel, mouse-pull thrust, camera rotation lock, the Nav Beacon autopilot, direct stat editing (speed/altitude/fuel/throttle/warp), noclip part placement, and an expanded build grid. These should all work relatively well except Nav Beacon LOL
